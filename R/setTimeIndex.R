...
set.time.index <- function(gldr.time.start, cmems.time.start, cov.time, train.length, nt) {
  cov.time.start <- gldr.time.start - as.numeric(as.POSIXct(cmems.time.start, tz = "GMT", origin="1970-01-01")) # converts gldr.mean start time to cov.time.start
  ...
  cov.predicting <- cov.training.end + 1:nt # when selecting from cov times
